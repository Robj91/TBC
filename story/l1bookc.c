void main(void)
{
 if (&sgchar == 1)
 {
  int &book = &current_sprite;
  script_attach(0);
  sp_script(&book, "l1bookcp2");
  kill_this_task();
 }
}
...
void talk(void)
{
 ...
 if (&x < 320)
 {
  ...
  set_y 240
  title_start();
  "The book case is filled with unorganised books, all stacked in a random order. There are a few interesting ones."
  title_end();
  ...
  choice_end();
    ...
    if (&result == 1)
    {
     say_stop("This book was written shortly after I defeated Seth", 1);
     wait(200);
     ...
     wait(200);
     say_stop("Uh... maybe I shouldn't have made that promise", 1);
    }
    if (&result == 2)
    {
     say_stop("The book is simply titled 'Stonebrook'", 1);
     wait(200);
    ...
    if (&result == 4)
    {
     ...
     wait(200);
     say_stop("I've always found this book.... quite interesting", 1);
    }
  unfreeze(1);
 }
 if (&x > 320)
 { 
  say("This bookcase is full of uninteresting books.", 1);
 }
}

void hit(void)
{
 say("Books must burn!", 1);
}   